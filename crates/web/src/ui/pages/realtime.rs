...
pub fn RealtimePanel(site_id: String) -> Element {
    ...
    use_future(move || async move {
        loop {
            // Poll only in the browser. During SSR the loop parks forever
            // (never ticks), so the initial render is emitted once and the
            // hydrated client takes over the live refresh.
    ...
    let rt = use_resource({
        ...
        move || {
            ...
            async move { get_json::<crate::ui::types::RealtimeSnapshot>(&path).await }
        }
    });
